...
export default function Register(){
  ...
  const handleSubmit = e => {
    e.preventDefault();
    if(!formData.name||!formData.email||!formData.password){ setMsg('Please fill out all fields.'); return; }
  ...
  return (
    <div className="form-box container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        {msg && <p className={msg.includes('successful') ? 'form-success' : 'form-error'}>{msg}</p>}
        <button className="form-button" type="submit">Register</button>
      </form>

      <Link to="/login" style={{display:'block', textAlign:'center', marginTop:14}}>Already have an account? Login here</Link>
    </div>
  );
}
